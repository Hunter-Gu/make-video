...
| Hybrid scenes | Partial | Image/video generation, reference frames, provenance, clip QA, Remotion video scenes, a cost estimate before a paid run, and resumable image and narration generation; an interrupted clip request is reported rather than silently re-billed, but the provider operation itself cannot be re-attached |
