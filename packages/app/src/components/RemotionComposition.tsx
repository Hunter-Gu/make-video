...
export const RemotionComposition = ({state}: {state: ProjectState}) => {
  ...
  const effect = scene ? state.effects.find((item) => item.sceneId === scene.id && frame >= item.startFrame && frame < item.endFrame) : null;
  ...
  return (
    <AbsoluteFill style={{background: '#090d14', color: '#f4ead7', fontFamily: 'Georgia, serif'}}>
      {asset?.url ? <Img src={asset.url} style={{width: '100%', height: '100%', objectFit: 'cover', transform: `scale(${1.03 + progress * .08})`}} /> : null}
