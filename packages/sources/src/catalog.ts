...
export const buildCatalog = (videoId: string, force: boolean) => {
  ...
  const index = JSON.parse(readFileSync(indexFile, "utf8")); const annotations = JSON.parse(readFileSync(annotationsFile, "utf8"));
  ...
  const claimsFile = resolve(context.sourceDir, "CLAIMS.json"); const claims = existsSync(claimsFile) ? JSON.parse(readFileSync(claimsFile, "utf8")).claims ?? [] : [];
