...
export const runGeneratedVideoQa = (args: string[]) => {
  ...
  for (const clip of config.clips ?? []) {
    ...
    if (clip.allowText !== true) {
      ...
      if (sample.status !== 0) throw new Error(`Could not sample ${clip.id} for text detection.`);
      detectedText = readImageText(sample.stdout as Buffer, clip.minOcrConfidence ?? 70);
    }
    const checks = {duration: Math.abs(duration - clip.expectedDurationSeconds) <= (clip.durationToleranceSeconds ?? .25), resolution: video?.width >= clip.minWidth && video?.height >= clip.minHeight, sceneCuts: sceneCuts <= (clip.maxSceneCuts ?? 0), unwantedText: detectedText.length === 0};
