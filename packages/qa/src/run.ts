...
export const runQa = (kind: QaKind, videoId: string, inputPath?: string) => new Promise<void>((resolvePromise, reject) => {
  ...
  const child = spawn(process.execPath, [script, kind, videoId], {cwd: process.cwd(), env: inputPath ? {...process.env, MAKE_VIDEO_QA_INPUT: inputPath} : process.env, stdio: "inherit"});
